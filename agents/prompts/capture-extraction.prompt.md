...
- changeDescription is a STANDARDISED restatement: one or two plain sentences, in the same register every time, saying what is being changed and where. Drop the greeting, the apology, the "kindly do the needful", the repetition, and anything about the sender. Keep every fact, every number the message itself contains, and every qualifier — "maybe", "the client mentioned", "not confirmed yet" — because a hedge removed is a claim strengthened, and that is not your call to make. Write it so a commercial manager reading forty of these can scan yours in three seconds. If the message is already one clear sentence, return it as it stands rather than dressing it up.
