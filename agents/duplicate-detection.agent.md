...
Suggest that a new capture may duplicate an existing Potential Change, so the same change is not raised twice.
...
`/agents/prompts/duplicate-detection.prompt.md` — not yet written.
